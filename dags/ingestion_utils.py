...
def _touched_partition_dirs(df: pl.DataFrame, base_path: str, partition_cols: list[str]) -> list[str]:
    '''Build the Hive-style partition directory paths actually written for this df.'''
    base = Path(base_path)
    touched = df.select(partition_cols).unique()
    return [
        str(base.joinpath(*(f"{col}={row[i]}" for i, col in enumerate(partition_cols))))
...
def write_bronze_partitioned(
    df: pl.DataFrame,
    partition_date_expr: pl.Expr,
    path: str,
    region_partitioned: bool = False,
) -> list[str]:
    '''Derive year/month/day Hive partitions from partition_date_expr and write,
    replacing any existing data in the touched partitions.

    Returns the touched partition directory paths (e.g. ".../year=2026/month=9/day=7"),
    scoped to just what this call wrote. Spark/pyarrow give every partition rewrite a
    new random filename, so a downstream Snowflake load must PUT exactly these
    directories rather than rescan the whole bronze tree, or a reprocessed day gets
    loaded twice.
    '''
    df = df.with_columns(
        year=partition_date_expr.dt.year(),
        month=partition_date_expr.dt.month(),
        day=partition_date_expr.dt.day(),
    )
    ...
    return _touched_partition_dirs(df, path, partition_cols)


def write_flat_staging_copy(df: pl.DataFrame, staging_path: str, filename: str) -> str:
    '''Write a flat (non-Hive) copy of df to staging_path, keeping partition columns
    as real parquet columns (Hive writers strip them into directory names only, and
    a Snowflake PUT flattens directory structure away, so this copy is what preserves
    them for the load).

    `filename` must be unique per call within a run (e.g. include the partition date
    and region) to avoid silently overwriting a previous staging file.
    '''

    Path(staging_path).mkdir(parents=True, exist_ok=True)
    flat_path = str(Path(staging_path) / filename)
